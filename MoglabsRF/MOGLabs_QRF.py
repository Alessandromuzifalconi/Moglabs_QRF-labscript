...
log_level = [logging.CRITICAL, logging.ERROR, logging.WARNING, logging.INFO, logging.DEBUG, logging.NOTSET][4]
...
class MOGLabs_QRF(PseudoclockDevice):
    ...
    # these values will be updated in add_device for fastes channels
...
class MOGLabs_QRF_Worker(Worker):
    ...
    def transition_to_buffered(self, device_name, h5file, initial_values, fresh):
        ...
        # ecah channel can be in table mode or not and in table mode can be triggered one time or each step
        ...
        #print(f" shot file {self.shot_file}")
        with h5py.File(self.shot_file, 'r') as hdf5_file:
            ...
            for ddsno in range(MAX_NUM_CHANNELS):
                table_name = 'TABLE_DATA%i'%ddsno
                if table_name in group:
                    table_data = group[table_name][:]  
                    #print('%s / channel %i data:' % (device_name, ddsno), table_data)
                ...
                for i, line in enumerate(data):
                    ...
                    if fresh or i >= len(oldtable) or (line['freq'], line['phase'], line['amp']) != (oldtable[i]['freq'], oldtable[i]['phase'], oldtable[i]['amp']):
